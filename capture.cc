...
uint8_t* yuyv2rgb(uint8_t* yuyv, uint32_t width, uint32_t height)
{
  ...
  for (size_t i = 0; i < height; i++) {
    for (size_t j = 0; j < width; j += 2) {
      ...
      rgb[index * 3 + 0] = y0 + 1.370705 * v;
      rgb[index * 3 + 1] = y0 + 0.698001 * v - 0.337633 * u;
      rgb[index * 3 + 2] = y0 + 1.732446 * u;
      rgb[index * 3 + 3] = y1 + 1.370705 * v;
      rgb[index * 3 + 4] = y1 + 0.698001 * v - 0.337633 * u;
      rgb[index * 3 + 5] = y1 + 1.732446 * u;
    }
  }
  return rgb;
}
